...
/* The ILI9341 needs a bunch of command/argument values to be initalized. They are stored in this struct.*/
typedef struct
{
	uint8_t cmd;
	uint8_t data[16];
...
void ili_data(spi_device_handle_t spi, const uint8_t *data, int len)
{
	...
	memset(&t, 0, sizeof(t)); //Zeor out the transaction
...
//This routine queues these commands up so they get send as quickly as possible.
static void send_line(spi_device_handle_t spi, int ypos, uint16_t *line)
{
	...
	//variables. We allocate them on the stack, so we need to re-onit them each call.
	for(x = 0; x < 6; x++) {
	...
	trans[3].tx_data[2] = (ypos+1)>>8; //End page hugh
...
static void display_pretty_colors(spi_device_handle_t spi)
{
	...
	while(1) {
		...
		for(y = 0; y < 240; y++) {
			...
			if(sendding_line!=-1) send_line_finish(spi);
			//Swap sending_line and calc_line
			sendding_line = calc_line;
			calc_line=(calc_line==1)?0:1;
			//Send the line we currently calculated.
			send_line(spi, y, line[sendding_line]);
